...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "reset")) {
    if(message.author.id === "261678719511429120") {
      ...
      message.channel.send("I'm sorry, VLV said that only he could do this... is there anything else I can do for you?");
    }
  }

  if (message.content.startsWith(prefix + "report")) {
    var firstAt = message.content.indexOf("!");
    var firstClose = message.content.indexOf(">");
    var lastAt = message.content.lastIndexOf("!");
    var lastClose = message.content.lastIndexOf(">");
    if(firstAt === lastAt) {
    ...
    var winnerID = message.content.substring(firstAt + 1, firstClose);
    var loserID = message.content.substring(lastAt + 1, lastClose);
    if(winnerID === loserID) {
    ...
    sql.get(`SELECT * FROM scores WHERE userId ="${winner.id}"`).then(row => {
      if (!row) {
        sql.run("INSERT INTO scores (userId, points) VALUES (?, ?)", [winner.id, 0]);
      } 
      else {
    ...
    sql.get(`SELECT * FROM scores WHERE userId ="${loser.id}"`).then(row => {
      if (!row) {
        sql.run("INSERT INTO scores (userId, points) VALUES (?, ?)", [loser.id, 0]);
      } 
      else {
